...
Windows 桌面工具箱。当前功能：应用快捷启动（拖动排序、项目属性、高清图标、拼音搜索）、截图（窗口识别、标注、贴图）、取色（HEX/RGB/HSL）、屏幕标尺、录屏（区域/窗口、系统声音与麦克风、暂停、裁剪首尾、导出 MP4/GIF）、文字识别（PaddleOCR 离线识别，中英文）、二维码识别与生成、可自定义的全局热键、数据备份与恢复（每日自动备份）、开机自启、托盘菜单、系统工具入口。
